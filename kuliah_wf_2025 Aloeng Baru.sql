...
CREATE DATABASE kuliah_wf_2025;
USE kuliah_wf_2025;
...
ALTER TABLE kategori_klinis 
MODIFY idkategori_klinis INT(11) NOT NULL AUTO_INCREMENT;
...
-- =====================================
-- MIGRATION MODUL 5 (Relasi sesuai alur baru)
-- =====================================
...
CREATE TABLE IF NOT EXISTS temu_dokter (
  ...
  idpet INT NOT NULL,
  idrole_user INT NOT NULL, -- siapa yang mendaftarkan (resepsionis)
  waktu_daftar DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  no_urut INT NOT NULL,
  status CHAR(1) DEFAULT 'M', -- M = Menunggu, P = Proses, S = Selesai
  FOREIGN KEY (idpet) REFERENCES pet(idpet),
  FOREIGN KEY (idrole_user) REFERENCES role_user(idrole_user)
);

-- 1. Hapus foreign key lama yang mengikat idpet di rekam_medis
ALTER TABLE rekam_medis DROP FOREIGN KEY fk_rekam_medis_pet1;

-- 2. Hapus kolom idpet dari rekam_medis
ALTER TABLE rekam_medis DROP COLUMN idpet;

-- 3. Tambahkan kolom idreservasi_dokter di rekam_medis
ALTER TABLE rekam_medis
  ADD COLUMN idreservasi_dokter INT NOT NULL,
  ADD CONSTRAINT fk_rekam_temudokter 
    FOREIGN KEY (idreservasi_dokter) REFERENCES temu_dokter(idreservasi_dokter);


-- 3. Pastikan AUTO_INCREMENT untuk PK utama

ALTER TABLE pet MODIFY idpet INT NOT NULL AUTO_INCREMENT;
ALTER TABLE rekam_medis MODIFY idrekam_medis INT NOT NULL AUTO_INCREMENT;
ALTER TABLE role_user MODIFY idrole_user INT NOT NULL AUTO_INCREMENT;
ALTER TABLE pemilik MODIFY idpemilik INT NOT NULL AUTO_INCREMENT;
...
CREATE TABLE perawat (
  id_perawat INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  alamat VARCHAR(100),
  no_hp VARCHAR(45),
  jenis_kelamin VARCHAR(1),
